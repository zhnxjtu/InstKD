...
class KDPointTrans(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        ''' feature-based KD'''
        self.feature_alignment_channel = spconv.SparseSequential(
            spconv.SubMConv3d(self.model_cfg.NUM_OUTPUT_FEATURES, self.model_cfg.NUM_OUTPUT_STUDENT, 3, padding=1,
                              bias=False, indice_key='fea_align'),
    ...
    def roi2voxel(self, rois, batch_size):
        with torch.no_grad():
            ...
            roi_grid_coords_x_max = (grid_xyz_max[:, :, 0:1] - self.point_cloud_range[0]) // self.voxel_size[0]
            roi_grid_coords_y_max = (grid_xyz_max[:, :, 1:2] - self.point_cloud_range[1]) // self.voxel_size[1]
            roi_grid_coords_z_max = (grid_xyz_max[:, :, 2:3] - self.point_cloud_range[2]) // self.voxel_size[2]
            # roi_grid_coords: (B, Nx6x6x6, 3)
            roi_grid_coords_max = torch.cat([roi_grid_coords_x_max, roi_grid_coords_y_max, roi_grid_coords_z_max, label], dim=-1).int()

            # compute the voxel coordinates of grid points
            roi_grid_coords_x_min = (grid_xyz_min[:, :, 0:1] - self.point_cloud_range[0]) // self.voxel_size[0]
            roi_grid_coords_y_min = (grid_xyz_min[:, :, 1:2] - self.point_cloud_range[1]) // self.voxel_size[1]
            roi_grid_coords_z_min = (grid_xyz_min[:, :, 2:3] - self.point_cloud_range[2]) // self.voxel_size[2]
            # roi_grid_coords: (B, Nx6x6x6, 3)
            roi_grid_coords_min = torch.cat([roi_grid_coords_x_min, roi_grid_coords_y_min, roi_grid_coords_z_min, label], dim=-1).int()
    ...
    def forward(self, batch_dict):
        ...
        if self.model_cfg.INSTANCE_WEIGHT.ENABLE:
            b, h, w, c = batch_dict['cls_preds_tea'].shape
            weight = self.KD_distance(batch_dict['cls_preds_stu'].view(b, h * w, c),
                                      batch_dict['cls_preds_tea'].view(b, h * w, c), 1.0)
            weight = weight.reshape(b, h, w)

        rois = batch_dict['gt_boxes'].detach()
        batch_size = batch_dict['batch_size']
        sample_stride = batch_dict['multi_scale_3d_strides'][self.model_cfg.ALIGN_SOURCE]
        stu_coords, stu_features, tea_coords, tea_features = \
            self.reorder_feature(batch_dict['multi_scale_3d_features'][self.model_cfg.ALIGN_SOURCE],
                                 batch_dict['multi_scale_3d_features_tea'][self.model_cfg.ALIGN_SOURCE])

        ''' FITNET for Channel alignment '''
        stu_fea_trans = batch_dict['multi_scale_3d_features'][self.model_cfg.ALIGN_SOURCE]
        stu_fea_alignment = self.feature_alignment_channel(stu_fea_trans)
        stu_coords_trans, stu_features_trans, _, _ = \
            self.reorder_feature(stu_fea_alignment, batch_dict['multi_scale_3d_features_tea'][self.model_cfg.ALIGN_SOURCE])
        ...
        cur_grid_max[:,:,0:3] = roi_grid_coords_max[:,:,0:3] // sample_stride
        cur_grid_min[:,:,0:3] = roi_grid_coords_min[:,:,0:3] // sample_stride
        ...
        for bs in range(batch_size):
            ...
            for label_count in range(high_bound[:,3].max()):
                ...
                for anchor_count in range(num_anchor):
                    ...
                    with torch.no_grad():
                        ...
                        idx_max_z = torch.nonzero(stu_coords[:, 1] <= cur_high_bound[anchor_count, 0]).squeeze()
                        idx_min_z = torch.nonzero(stu_coords[:, 1] >= cur_low_bound[anchor_count, 0]).squeeze()
                        ...
                        idx_max_x = torch.nonzero(stu_coords[:, 3] <= cur_high_bound[anchor_count, 2]).squeeze()
                        idx_min_x = torch.nonzero(stu_coords[:, 3] >= cur_low_bound[anchor_count, 2]).squeeze()
                    ...
                    if self.model_cfg.INSTANCE_WEIGHT.ENABLE:
                        ...
                        if idx_obj.numel():
                            ...
                            cur_weight = weight[bs, mask_coords[:,0].long(), mask_coords[:,1].long()].mean()
        ...
        if self.model_cfg.RELA_KD.ENABLE:
            ...
            for bs in range(batch_size):
                ...
                for cur_label in range(max_label):
                    # teacher relation
                    cur_label_tea_matrix = cur_tea_matrix[cur_label]
                    idx_keep = torch.tensor([i for i in range(max_label) if i != cur_label])
                    cur_other_tea_matrix = torch.index_select(cur_tea_matrix, 0, idx_keep.cuda())

                    cur_label_tea_matrix_resize = cur_label_tea_matrix.view(-1, cur_tea_list_total.shape[1]).unsqueeze(
                        0).repeat(max_anchor_num, 1, 1)
                    cur_other_tea_matrix_resize = cur_other_tea_matrix.view(-1, cur_tea_list_total.shape[1]).unsqueeze(
                        0).repeat(max_anchor_num, 1, 1)
                    ...
                    intri_tea = intri_tea.permute(0, 2, 1).reshape(-1, max_num_point)
                    inter_tea = inter_tea.permute(0, 2, 1).reshape(-1, max_num_point)
                    ...
                    # student relation
                    cur_label_stu_matrix = cur_stu_matrix[cur_label]
                    cur_other_stu_matrix = torch.index_select(cur_stu_matrix, 0, idx_keep.cuda())

                    cur_label_stu_matrix_resize = cur_label_stu_matrix.view(-1, cur_pure_stu_list_total.shape[1]).unsqueeze(
                        0).repeat(max_anchor_num, 1, 1)
                    cur_other_stu_matrix_resize = cur_other_stu_matrix.view(-1, cur_pure_stu_list_total.shape[1]).unsqueeze(
                        0).repeat(max_anchor_num, 1, 1)
                    ...
                    intri_stu = intri_stu.permute(0, 2, 1).reshape(-1, max_num_point)
                    inter_stu = inter_stu.permute(0, 2, 1).reshape(-1, max_num_point)
            ...
            rela_intri_stu_total = F.interpolate(rela_intri_stu_total.unsqueeze(1), self.model_cfg.NUM_FEATURES).squeeze()
            rela_inter_stu_total = F.interpolate(rela_inter_stu_total.unsqueeze(1), self.model_cfg.NUM_FEATURES).squeeze()
            rela_intri_tea_total = F.interpolate(rela_intri_tea_total.unsqueeze(1), self.model_cfg.NUM_FEATURES).squeeze()
            rela_inter_tea_total = F.interpolate(rela_inter_tea_total.unsqueeze(1), self.model_cfg.NUM_FEATURES).squeeze()
